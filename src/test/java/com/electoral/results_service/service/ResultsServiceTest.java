...
class ResultsServiceTest {
    ...
    void setUp() {
        ...
        mappedResponse = ResultsResponse.builder()
                .electionId(1L)
                .totalVotes(11500000)
                .candidates(List.of(
    ...
    @Test
    @DisplayName("F-01 | EQ-2 | Los datos de resultados no son alterados entre BD y respuesta")
    void should_notAlterResultData_when_mappingFromDatabase() {
        ...
        when(mapper.toResponse(1L, presidencialResults)).thenReturn(mappedResponse);
    ...
    @Test
    @DisplayName("F-02 | EQ-1 | La respuesta incluye todos los campos requeridos")
    ...
    // F-03 | EQ-1 | Ningún campo nulo
    @Test
    @DisplayName("F-03 | EQ-1 | Ningún campo llega nulo para elección con resultados")
    ...
    @Test
    @DisplayName("F-04 | EQ-2 | El electionId retornado coincide exactamente con el consultado")
    ...
    // F-05 | EQ-3 | Doble Verdad: total === suma individual
    @Test
    @DisplayName("F-05 | EQ-3 | Doble Verdad: totalVotes coincide con suma de votos individuales")
    void should_matchTotalWithIndividualSum_when_calculatingResults() {
        ...
        int sumaManual = result.getCandidates().stream()
                .mapToInt(CandidateResult::getVotes).sum();
        assertEquals(sumaManual, result.getTotalVotes());
    }
 
    // F-06 | EQ-2 | El mapper es llamado con electionId y lista correctos
    @Test
    @DisplayName("F-06 | EQ-2 | El mapper recibe electionId y lista de resultados correctos")
    void should_callMapperWithCorrectArgs_when_queryingDatabase() {
        when(cache.get("results:1")).thenReturn(null);
        when(repository.findByElectionId(1L)).thenReturn(presidencialResults);
        when(mapper.toResponse(1L, presidencialResults)).thenReturn(mappedResponse);
        ...
        verify(mapper, times(1)).toResponse(1L, presidencialResults);
    }
 
    // ── RENDIMIENTO ───────────────────────────────────────────
 
    // R-01 | EQ-4 | CACHE HIT: ObjectMapper convierte y retorna sin tocar BD
    @Test
    @DisplayName("R-01 | EQ-4 | CACHE HIT: convierte con ObjectMapper y no consulta la BD")
    void should_returnFromCache_when_resultsAreCached() {
        when(cache.get("results:1")).thenReturn(mappedResponse);
        when(objectMapper.convertValue(mappedResponse, ResultsResponse.class))
        ...
        verify(mapper, never()).toResponse(anyLong(), anyList());
    }
 
    // R-02 | EQ-4 | CACHE MISS: almacena en caché tras consulta a BD
    @Test
    @DisplayName("R-02 | EQ-4 | CACHE MISS: almacena resultado en caché tras consulta a BD")
    ...
    // R-03 | EQ-4 | ObjectMapper falla al convertir: cae a BD correctamente
    @Test
    @DisplayName("R-03 | EQ-4 | Si ObjectMapper falla al convertir caché, consulta BD correctamente")
    void should_fallbackToDatabase_when_objectMapperFails() {
        when(cache.get("results:1")).thenReturn("valor_corrupto");
        when(objectMapper.convertValue("valor_corrupto", ResultsResponse.class))
                .thenThrow(new IllegalArgumentException("No se puede convertir"));
        when(repository.findByElectionId(1L)).thenReturn(presidencialResults);
        when(mapper.toResponse(1L, presidencialResults)).thenReturn(mappedResponse);
    ...
    // R-04 | EQ-6 | Consultas masivas: caché evita múltiples accesos a BD
    @Test
    @DisplayName("R-04 | EQ-6 | Consultas masivas: caché evita sobrecarga a BD al cierre electoral")
    void should_supportMassiveLoad_when_cacheIsWarmed() {
        when(cache.get("results:1")).thenReturn(mappedResponse);
        when(objectMapper.convertValue(mappedResponse, ResultsResponse.class))
                .thenReturn(mappedResponse);
    ...
    @Test
    @DisplayName("S-01 | EQ-19 | Elección inexistente lanza excepción sin exponer datos de otras")
    ...
    @Test
    @DisplayName("S-02 | EQ-20 | Ningún candidato tiene votos negativos (integridad)")
    void should_haveNoNegativeVotes_when_processingResults() {
        ...
        result.getCandidates().forEach(c ->
            assertTrue(c.getVotes() >= 0,
                c.getName() + " no puede tener votos negativos"));
    }
 
    // S-03 | EQ-20 | Sin contaminación cruzada entre elecciones
    @Test
    @DisplayName("S-03 | EQ-20 | Sin contaminación cruzada entre resultados de diferentes elecciones")
    void should_isolateResultsByElection_when_multipleElectionsExist() {
        ...
        ResultsResponse senadoResponse = ResultsResponse.builder()
                .electionId(2L).totalVotes(2200000)
                .candidates(List.of(new CandidateResult("Partido A", 2200000)))
                .build();
 
        when(cache.get("results:2")).thenReturn(null);
        when(repository.findByElectionId(2L)).thenReturn(senado);
        when(mapper.toResponse(2L, senado)).thenReturn(senadoResponse);
 
        ResultsResponse result = service.getResults(2L);
 
        assertEquals(2L, result.getElectionId());
        assertFalse(result.getCandidates().stream()
                .anyMatch(c -> c.getName().equals("Juan Pérez")));
    }
 
    // S-04 | EQ-21 | Repositorio consultado exactamente una vez
    @Test
    @DisplayName("S-04 | EQ-21 | El repositorio es consultado exactamente una vez por solicitud")
    ...
    // S-05 | EQ-21 | Caché escrita exactamente una vez
    @Test
    @DisplayName("S-05 | EQ-21 | La caché se escribe exactamente una vez para evitar duplicación")
    ...
    // A-01 | EQ-31 | Genera clave de caché trazable con electionId
    @Test
    @DisplayName("A-01 | EQ-31 | Genera clave de caché trazable con el electionId")
    void should_generateTraceableCacheKey_when_electionQueried() {
        ...
        verify(cache).get("results:1");
        verify(cache).set(eq("results:1"), any());
    }
 
    // A-02 | EQ-31 | Con CACHE HIT la BD no es consultada
    @Test
    @DisplayName("A-02 | EQ-31 | Con CACHE HIT queda rastro de que la BD no fue consultada")
    ...
    @Test
    @DisplayName("A-03 | EQ-25 | Las capas de caché, repositorio y mapper son verificables independientemente")
    void should_allowIndependentLayerVerification_when_auditing() {
        ...
        verify(cache, times(1)).get("results:1");
        verify(repository, times(1)).findByElectionId(1L);
        verify(mapper, times(1)).toResponse(1L, presidencialResults);
        verify(cache, times(1)).set(eq("results:1"), any());
    }
